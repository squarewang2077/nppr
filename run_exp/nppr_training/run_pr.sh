#!/bin/bash
# run_pr.sh
# Local-Entropy Probabilistic Robustness (PR) training of
# ResNet18 / ResNet50 / WideResNet50-2 / VGG16 on
# CIFAR-10, CIFAR-100, and TinyImageNet — WITH data augmentation.
#
# Identical to run_pr_noAug.sh except the train loader uses augmentation
# (RandomCrop + HorizontalFlip + RandAugment + RandomErasing). Output
# filenames are tagged with '_Aug' (by the trainer for the transitional
# files, and by this script in the final renamed RUN_NAME) so they do
# not collide with the no-aug checkpoints.
#
# Sweeps over ANCHOR_LAMBDA / NUM_STARTS / NUM_STEPS; each run's outputs
# are renamed to
#   level_t<T_LEVEL>_S<NUM_STEPS>_A<ANCHOR_LAMBDA>_N<NUM_STARTS>_Aug.{pth,log,csv}
# so that the sweep does not clobber prior runs.

set -euo pipefail

# Resolve the project root (two levels up from this script) and add it to
# PYTHONPATH so that top-level packages (arch/, src/, utils/, configs/) are
# importable. cd ensures ./dataset and scripts/... resolve correctly no
# matter where this script is invoked from.
PROJECT_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
export PYTHONPATH="${PROJECT_ROOT}:${PYTHONPATH:-}"
cd "${PROJECT_ROOT}"

# ---------------------------------------------------------------------------
# Configurable hyper-parameters
# ---------------------------------------------------------------------------
GPU_ID=1                     # GPU device ID to use (change this to 0, 1, 3, etc.)
export CUDA_VISIBLE_DEVICES=${GPU_ID}

DATA_ROOT="./dataset"
EPOCHS=100
BATCH_SIZE=256
LR=0.01
WEIGHT_DECAY=5e-4
SEED=42

ARCHS=("resnet50" "wide_resnet50_2")
# ARCHS=("resnet18" "resnet50" "wide_resnet50_2" "vgg16")
# DATASETS=("cifar10")
DATASETS=("cifar10" "cifar100" "tinyimagenet")
TRAINING_TYPE="level"  # standard | level

# Perturbation budget
NORM="linf"
EPSILON=0.03137              # 8/255

# Particle settings
NUM_STARTS_LIST=(8)

# Langevin dynamics
NUM_STEPS_LIST=50
STEP_SIZE=1e-2

# Energy function
TOL=0.05                     # |m - t| <= TOL counts as valid
PSI_ALPHA=10.0

# Threshold strategy
T_LEVEL=0.0                   # target margin level

# Scope strategy

# GAMMA sweep
ANCHOR_LAMBDAS=(0.02)        # L2 pull toward each random start

# Outer TRADES-style loss

# Save root
SAVE_ROOT="./ckp/nppr_training/pr_training"

# ---------------------------------------------------------------------------
# Training loop
# ---------------------------------------------------------------------------
echo "======================================================"
echo "  Using GPU: ${GPU_ID}"
echo "  Training Type: ${TRAINING_TYPE}  (with data augmentation)"
echo "  ANCHOR_LAMBDA sweep: ${ANCHOR_LAMBDAS[*]}"
echo "  NUM_STARTS sweep: ${NUM_STARTS_LIST[*]}"
echo "  NUM_STEPS sweep: ${NUM_STEPS_LIST[*]}"
echo "======================================================"
echo ""

for DATASET in "${DATASETS[@]}"; do
    for ARCH in "${ARCHS[@]}"; do
        SAVE_DIR="${SAVE_ROOT}/${DATASET}/${ARCH}/${TRAINING_TYPE}"
        mkdir -p "${SAVE_DIR}"

        for NUM_STARTS in "${NUM_STARTS_LIST[@]}"; do
        for NUM_STEPS in "${NUM_STEPS_LIST[@]}"; do
        for ANCHOR_LAMBDA in "${ANCHOR_LAMBDAS[@]}"; do
            RUN_NAME="level_t${T_LEVEL}_S${NUM_STEPS}_A${ANCHOR_LAMBDA}_N${NUM_STARTS}_Aug"
            echo "======================================================"
            echo "  arch=${ARCH}  dataset=${DATASET}  t=${T_LEVEL}  num_starts=${NUM_STARTS}  num_steps=${NUM_STEPS}  augmentation=ON"
            echo "  run=${RUN_NAME}"
            echo "  save_dir=${SAVE_DIR}"
            echo "======================================================"

            python scripts/pr_training/pos_geo_training.py \
                --dataset        "${DATASET}"        \
                --data_root      "${DATA_ROOT}"      \
                --arch           "${ARCH}"           \
                --training_type  "${TRAINING_TYPE}"  \
                --augment                             \
                --epochs         "${EPOCHS}"         \
                --batch_size     "${BATCH_SIZE}"     \
                --lr             "${LR}"             \
                --weight_decay   "${WEIGHT_DECAY}"   \
                --seed           "${SEED}"           \
                --norm           "${NORM}"           \
                --epsilon        "${EPSILON}"        \
                --num_starts     "${NUM_STARTS}"      \

                --num_steps      "${NUM_STEPS}"       \

                --step_size      "${STEP_SIZE}"       \

                --t              "${T_LEVEL}"         \

                --anchor_lambda  "${ANCHOR_LAMBDA}"   \

                --psi_alpha      "${PSI_ALPHA}"       \

                --tol            "${TOL}"             \
                --eval_pgd --pgd_steps 10 --pgd_norm linf \
                --eval_level --level_n 8 --level_steps 50 --level_norm linf \
                --eval_random --random_n 8 --random_norm linf \
                --random_dist gaussian uniform laplace \
                --save_dir       "${SAVE_DIR}"

            # Rename outputs so each sweep run keeps its own files.
            # With --augment the trainer writes:
            #   <save_dir>/<arch_lower>_<dataset_lower>_<training_type>_Aug.{pth,log}
            #   <save_dir>/<arch_lower>_<dataset_lower>_<training_type>_Aug_training_info.csv
            ARCH_LOWER="${ARCH,,}"
            DATASET_LOWER="${DATASET,,}"
            SRC_BASE="${SAVE_DIR}/${ARCH_LOWER}_${DATASET_LOWER}_${TRAINING_TYPE}_Aug"
            DST_BASE="${SAVE_DIR}/${RUN_NAME}"

            mv "${SRC_BASE}.pth"                  "${DST_BASE}.pth"
            mv "${SRC_BASE}.log"                  "${DST_BASE}.log"
            mv "${SRC_BASE}_training_info.csv"    "${DST_BASE}_training_info.csv"

            echo "  -> renamed outputs to ${RUN_NAME}.{pth,log,csv}"
        done
        done
        done
    done
done

echo ""
echo "All ${TRAINING_TYPE} (aug) training runs completed."
