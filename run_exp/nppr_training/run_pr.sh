...
# CIFAR-10, CIFAR-100, and TinyImageNet — WITH data augmentation.
#
# Identical to run_pr_noAug.sh except the train loader uses augmentation
# (RandomCrop + HorizontalFlip + RandAugment + RandomErasing). Output
# filenames are tagged with '_Aug' (by the trainer for the transitional
# files, and by this script in the final renamed RUN_NAME) so they do
# not collide with the no-aug checkpoints.
#
# Sweeps over GAMMA / NUM_PARTICLES / LANGEVIN_STEPS; each run's outputs
# are renamed to
#   loc_ent_eps<LANGEVIN_STEPS>_L<LANGEVIN_BETA>_G<GAMMA>_N<NUM_PARTICLES>_Aug.{pth,log,csv}
# so that the sweep does not clobber prior runs.
...
for DATASET in "${DATASETS[@]}"; do
    for ARCH in "${ARCHS[@]}"; do
        ...
        for GAMMA in "${GAMMAS[@]}"; do
            RUN_NAME="loc_ent_eps${LANGEVIN_STEPS}_L${LANGEVIN_BETA}_G${GAMMA}_N${NUM_PARTICLES}_Aug"
            echo "======================================================"
            echo "  arch=${ARCH}  dataset=${DATASET}  gamma=${GAMMA}  num_particles=${NUM_PARTICLES}  langevin_steps=${LANGEVIN_STEPS}  augmentation=ON"
            echo "  run=${RUN_NAME}"
            ...
            # Rename outputs so each sweep run keeps its own files.
            # With --augment the trainer writes:
            #   <save_dir>/<arch_lower>_<dataset_lower>_<training_type>_Aug.{pth,log}
            #   <save_dir>/<arch_lower>_<dataset_lower>_<training_type>_Aug_training_info.csv
            ARCH_LOWER="${ARCH,,}"
            DATASET_LOWER="${DATASET,,}"
            SRC_BASE="${SAVE_DIR}/${ARCH_LOWER}_${DATASET_LOWER}_${TRAINING_TYPE}_Aug"
